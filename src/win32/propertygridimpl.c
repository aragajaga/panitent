...
static LPTSTR NewStringArray(LPTSTR szzStr)
{
    ...
    //Determine total pszBuffer length
    INT iLen = 0;
    //Walk the pszBuffer to find the terminating empty string.
...
static LPLISTBOXITEM NewItem(LPTSTR szCatalog, LPTSTR szPropName, LPTSTR szCurValue, 
    LPTSTR szMisc, LPTSTR szPropDesc, INT iItemType, LPVOID lpUserData)
{
...
///
/// @returns VOID.
VOID Editor_DoUpdate(HWND hwnd, HWND hwndCtl, BOOL fExitEd)
{
...
static VOID EditOwner_OnNotify(HWND hwnd, INT id, HWND hwndCtl, UINT codeNotify)
{
	switch(id)
	{
		case ID_EDIT:
			...
			{
				Editor_DoUpdate(hwnd,hwndCtl, TRUE);
			}
	}
}
...
static VOID FileDialogItem_FromString(LPPROPGRIDFDITEM lpPgFdItem, LPTSTR lpszFdItem)
{
    ...
    _stscanf(lpszFdItem, 
        _T( "Title: %256")_T(SCN_SPEC)_T("[^\r\n] "
            ...
            "Default Extension: %3")_T(STR_SPEC), 
        (LPTSTR) &PgFdItem[0], 
        (LPTSTR) &PgFdItem[1], 
        (LPTSTR) &PgFdItem[2], 
        (LPTSTR) &PgFdItem[3]);
...
///
/// @param dest  Pointer to a pszBuffer that will contain the converted string.
/// @param ccDest  destination pszBuffer size (number of characters)
...
///
/// @param hwnd The handle to this control (where circular pszBuffer stored)
/// @param str1 A string
...
LPTSTR Join(HWND hwnd, LPTSTR str1, LPTSTR str2)
{
    ...
    if (NULL == ppStoriage)
    {
        // Create and store a circular pszBuffer
...
HBRUSH ComboBox_OnCtlColorListbox(HWND hwnd, HDC hdc, HWND hwndChild, int type)
{
    ...
    if (CBS_OWNERDRAWFIXED & GetWindowLongPtr(hwnd, GWL_STYLE))// checked listbox
    {
...
/// @param lpszText Points to the pszBuffer that is to receive the text.
///
        ...
        if (NULL != g_lpInst->lpCurrent)
        {
            switch (g_lpInst->lpCurrent->iItemType)
            {
                ...
                case PIT_DATETIME:
				{
                    ...
                    break;	
                default:
					Editor_DoUpdate(hwnd, g_lpInst->hwndCtl1, TRUE);
                    break;
            }
        }
...
static VOID ListBox_OnMouseMove(HWND hwnd, INT x, INT y, UINT keyFlags)
{
    ...
    if (NULL != g_lpInst->hwndToolTip)
    {
        ...
        SendMessage(g_lpInst->hwndToolTip, 0, (WPARAM)0, (LPARAM)(LPTOOLINFO)&tiToolInfo);
            ...
            //In order to reduce flicker, Do not update unless text changed
        {
            ...
            SendMessage(g_lpInst->hwndToolTip, TTM_UPDATETIPTEXT, 0, (LPARAM)(LPTOOLINFO)&tiToolInfo);
        }
    }
}
...
static VOID Grid_OnSize(HWND hwnd, UINT state, INT cx, INT cy)
{
    ...
    if (NULL != g_lpInst->hwndToolTip)  //Resize ToolTips
    {
        ...
        SendMessage(g_lpInst->hwndToolTip, TTM_NEWTOOLRECT, 0, (LPARAM)(LPTOOLINFO)&ti);
    }
    if (NULL != g_lpInst->lpCurrent)
    {
        if (PIT_CHECK == g_lpInst->lpCurrent->iItemType)
...
static INT Grid_OnAddString(LPPROPGRIDITEM pgi)
{
    ...
    {
        lpi = NewItem(pgi->lpszCatalog, _T(""), _T(""), _T(""), _T(""), PIT_CATALOG, NULL);
        lpi->fCollapsed = TRUE;
