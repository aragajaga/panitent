...
LRESULT CALLBACK option_bar_wndproc(HWND hwnd, UINT message, WPARAM wparam,
    LPARAM lparam)
{
  switch (message)
  {
...
void option_bar_register_class(HINSTANCE hInstance)
{
  ...
  if (!class_atom) {
    MessageBox(NULL, L"Failed to register option bar class!", NULL,
        MB_OK | MB_ICONERROR);
    return;
  }
...
{
  HWND handle = CreateWindowEx(0, MAKEINTATOM(g_option_bar.win_class),
      NULL, WS_CHILD | WS_VISIBLE, 0, 0, 500, 64, hwnd, NULL,
      GetModuleHandle(NULL), NULL);

  if (!handle)
  {
    MessageBox(NULL, L"Failed to create option bar window!", NULL,
        MB_OK | MB_ICONERROR);
    return;
  }
