...
#include "../src/dock/host/dockhost.h"
#include "../src/dock/host/dockhostdrag.h"
#include "../src/dock/host/dockhosttoolapply.h"
#include "../src/dock/layout/docklayout.h"
#include "../src/dock/document/documentdocktransition.h"
#include "../src/dock/floating/dockfloatingmodel.h"
#include "../src/dock/floating/dockfloatingpersist.h"
#include "../src/dock/floating/floatingchildhost.h"
#include "../src/dock/floating/floatingdocumentcreate.h"
#include "../src/dock/floating/floatingdocumentlayoutmodel.h"
#include "../src/dock/floating/floatingdocumentlayoutpersist.h"
#include "../src/dock/floating/floatingdocumentsessionpersist.h"
#include "../src/dock/floating/floatingwindowcontainer.h"
#include "../src/resource.h"
#include "../src/dock/layout/windowlayoutcatalog.h"
#include "../src/canvas.h"
#include "../src/document.h"
#include "../src/documentrecovery.h"
#include "../src/dock/model/dockmodel.h"
#include "../src/dock/model/dockmodelops.h"
#include "../src/dock/view/dockshell.h"
#include "../src/dock/floating/floatingdocumentsessionmodel.h"
#include "../src/panitentapp.h"
#include "../src/panitentwindow.h"
#include "../src/workspacecontainer.h"
#include "../src/dock/layout/windowlayoutmanager.h"
#include "../src/dock/layout/windowlayoutprofile.h"
...
typedef struct RuntimeGuiResourceSnapshot
{
    DWORD nUserObjects;
    DWORD nGdiObjects;
} RuntimeGuiResourceSnapshot;
...
static RuntimeGuiResourceSnapshot runtime_capture_gui_resources(void)
{
    RuntimeGuiResourceSnapshot snapshot = { 0 };
    snapshot.nUserObjects = GetGuiResources(GetCurrentProcess(), GR_USEROBJECTS);
    snapshot.nGdiObjects = GetGuiResources(GetCurrentProcess(), GR_GDIOBJECTS);
    return snapshot;
}
...
static int test_runtime_repeated_floating_document_session_strict_failure_cycles_remain_stable(void)
{
    DockRuntimeFixture fixture = { 0 };
    assert(runtime_fixture_init(&fixture));
    ...
    Canvas* pCanvas = Canvas_Create(16, 16);
    assert(pCanvas != NULL);
    Document* pDocument = Document_CreateWithCanvas(pCanvas);
    assert(pDocument != NULL);
    assert(Document_AttachToWorkspace(pDocument, pMainWorkspace));
    ...
    Document* pRecoveryDocA = Document_CreateWithCanvas(Canvas_Create(12, 12));
    Document* pRecoveryDocB = Document_CreateWithCanvas(Canvas_Create(20, 20));
    assert(pRecoveryDocA != NULL);
    assert(pRecoveryDocB != NULL);
    assert(DocumentRecovery_Save(pRecoveryDocA, szRecoveryA));
    assert(DocumentRecovery_Save(pRecoveryDocB, szRecoveryB));
    ...
    pSessionModel->entries[0].pLayoutModel = &rootA;
    pSessionModel->entries[0].nWorkspaceCount = 1;
    pSessionModel->entries[0].workspaces[0].uWorkspaceNodeId = 701;
    pSessionModel->entries[0].workspaces[0].nActiveEntry = 0;
    pSessionModel->entries[0].workspaces[0].nFileCount = 1;
    pSessionModel->entries[0].workspaces[0].entries[0].nKind = DOCSESSION_ENTRY_RECOVERY;
    wcscpy_s(
    ...
    pSessionModel->entries[1].pLayoutModel = &rootB;
    pSessionModel->entries[1].nWorkspaceCount = 1;
    pSessionModel->entries[1].workspaces[0].uWorkspaceNodeId = 702;
    pSessionModel->entries[1].workspaces[0].nActiveEntry = 0;
    pSessionModel->entries[1].workspaces[0].nFileCount = 1;
    pSessionModel->entries[1].workspaces[0].entries[0].nKind = DOCSESSION_ENTRY_RECOVERY;
    wcscpy_s(
...
static int test_runtime_repeated_menu_command_save_overwrite_cycles_remain_stable(void)
{
    DockRuntimeFixture fixture = { 0 };
    assert(runtime_fixture_init(&fixture));
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        if (uLayoutId == 0)
        {
            uLayoutId = catalog.entries[0].uId;
        }
        assert(catalog.entries[0].uId == uLayoutId);

        DockModelNode* pLoadedLayout = NULL;
        DockFloatingLayoutFileModel loadedFloating = { 0 };
        FloatingDocumentLayoutModel loadedFloatDocs = { 0 };
        assert(WindowLayoutProfile_LoadBundle(uLayoutId, &pLoadedLayout, &loadedFloating, &loadedFloatDocs));
        assert(pLoadedLayout != NULL);
        ...
        if ((i % 2) == 0)
        {
            ...
            assert(loadedFloatDocs.nEntryCount == 0);
            assert(runtime_model_subtree_contains_name(pRightZone, L"GLWindow"));
        }
        else
        {
    ...
    g_runtimeWindowLayoutPromptName[0] = L'\0';
    DockModel_Destroy(pMixedLayout);
    runtime_delete_profile_bundle(uLayoutId);
    runtime_delete_window_layout_catalog_file();
    runtime_fixture_destroy(&fixture);
    return 0;
}
...
static int test_runtime_repeated_mixed_layout_cycles_with_session_restore_remain_stable(void)
{
    DockRuntimeFixture fixture = { 0 };
    assert(runtime_fixture_init(&fixture));
    ...
    HWND hWndWorkspaceBefore = runtime_get_live_hwnd_by_name(fixture.pDockHostWindow, L"WorkspaceContainer");
    assert(hWndWorkspaceBefore && IsWindow(hWndWorkspaceBefore));
    WorkspaceContainer* pMainWorkspace = (WorkspaceContainer*)WindowMap_Get(hWndWorkspaceBefore);
    assert(pMainWorkspace != NULL);

    Canvas* pCanvas = Canvas_Create(20, 20);
    assert(pCanvas != NULL);
    Document* pDocument = Document_CreateWithCanvas(pCanvas);
    assert(pDocument != NULL);
    assert(Document_AttachToWorkspace(pDocument, pMainWorkspace));
    ...
    DockFloatingLayoutFileModel floatingB = { 0 };
    floatingB.nEntries = 1;
    SetRect(&floatingB.entries[0].rcWindow, 160, 150, 460, 480);
    floatingB.entries[0].iDockSizeHint = 240;
    floatingB.entries[0].nChildKind = FLOAT_DOCK_CHILD_TOOL_PANEL;
    floatingB.entries[0].nViewId = PNT_DOCK_VIEW_GLWINDOW;
    ...
    FloatingDocumentLayoutModel floatDocsB = { 0 };
    floatDocsB.nEntryCount = 1;
    SetRect(&floatDocsB.entries[0].rcWindow, 520, 180, 900, 620);
    floatDocsB.entries[0].pLayoutModel = &floatDocRoot;
...
static int test_runtime_repeated_direct_floating_failure_cycles_remain_stable(void)
{
    DockRuntimeFixture fixture = { 0 };
    assert(runtime_fixture_init(&fixture));
    ...
    DockFloatingLayoutFileModel floatingModel = { 0 };
    floatingModel.nEntries = 2;
    SetRect(&floatingModel.entries[0].rcWindow, 180, 140, 500, 470);
    floatingModel.entries[0].iDockSizeHint = 240;
    floatingModel.entries[0].nChildKind = FLOAT_DOCK_CHILD_TOOL_PANEL;
    floatingModel.entries[0].nViewId = PNT_DOCK_VIEW_GLWINDOW;
    SetRect(&floatingModel.entries[1].rcWindow, 520, 140, 840, 470);
    floatingModel.entries[1].iDockSizeHint = 240;
    floatingModel.entries[1].nChildKind = FLOAT_DOCK_CHILD_TOOL_PANEL;
    floatingModel.entries[1].nViewId = PNT_DOCK_VIEW_PALETTE;
    ...
    SetRect(&floatDocModel.entries[0].rcWindow, 560, 180, 940, 620);
    floatDocModel.entries[0].pLayoutModel = &floatDocRoot;
    SetRect(&floatDocModel.entries[1].rcWindow, 960, 200, 1260, 560);
    floatDocModel.entries[1].pLayoutModel = &floatDocRoot;
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        g_runtimeDockFloatingRestoreFailCountdown = 1;
        PanitentDockFloating_SetRestoreEntryTestHook(runtime_fail_floating_tool_restore_on_countdown);
        assert(!PanitentDockFloating_RestoreModelEx(fixture.pApp, fixture.pDockHostWindow, &floatingModel, TRUE));
        PanitentDockFloating_SetRestoreEntryTestHook(NULL);
        g_runtimeDockFloatingRestoreFailCountdown = -1;
        ...
        DockModelNode* pAfterToolFail = DockModel_CaptureHostLayout(fixture.pDockHostWindow);
        DockModelNode* pRightZoneAfterToolFail = runtime_find_model_zone(pAfterToolFail, DKS_RIGHT);
        assert(pAfterToolFail != NULL);
        assert(pRightZoneAfterToolFail != NULL);
        assert(!runtime_model_subtree_contains_name(pRightZoneAfterToolFail, L"GLWindow"));
        DockModel_Destroy(pAfterToolFail);
        ...
        DockModelNode* pAfterDocFail = DockModel_CaptureHostLayout(fixture.pDockHostWindow);
        DockModelNode* pRightZoneAfterDocFail = runtime_find_model_zone(pAfterDocFail, DKS_RIGHT);
        assert(pAfterDocFail != NULL);
        assert(pRightZoneAfterDocFail != NULL);
        assert(!runtime_model_subtree_contains_name(pRightZoneAfterDocFail, L"GLWindow"));
        DockModel_Destroy(pAfterDocFail);
    }
...
static int test_runtime_repeated_save_apply_reset_cycles_remain_stable(void)
{
    DockRuntimeFixture fixture = { 0 };
    assert(runtime_fixture_init(&fixture));
...
static int test_runtime_save_apply_reset_cycles_do_not_drift_gui_resources(void)
{
    DockRuntimeFixture fixture = { 0 };
    assert(runtime_fixture_init(&fixture));
...
static int test_runtime_mixed_layout_cycles_do_not_drift_gui_resources(void)
{
    DockRuntimeFixture fixture = { 0 };
    assert(runtime_fixture_init(&fixture));
    ...
    assert(WindowLayoutManager_ApplyLayoutBundle(&fixture.panitentWindow, pTarget, &floatingModel, &floatDocModel));
    assert(WindowLayoutManager_ApplyDefaultLayout(&fixture.panitentWindow));
    ...
    RuntimeGuiResourceSnapshot final = runtime_capture_gui_resources();
    assert(final.nUserObjects <= baseline.nUserObjects + 32);
    assert(final.nGdiObjects <= baseline.nGdiObjects + 32);
...
static int test_runtime_repeated_command_failure_cycles_from_mixed_state_remain_stable(void)
{
    DockRuntimeFixture fixture = { 0 };
    assert(runtime_fixture_init(&fixture));

    const uint32_t uIdA = 0x86000000u | (uint32_t)(GetCurrentProcessId() & 0x0000FFFFu);
    const uint32_t uIdB = uIdA + 1u;
    runtime_delete_profile_bundle(uIdA);
    runtime_delete_profile_bundle(uIdB);
    runtime_delete_window_layout_catalog_file();
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        DockModelNode* pAfterFail = DockModel_CaptureHostLayout(fixture.pDockHostWindow);
        DockModelNode* pRightZoneAfterFail = runtime_find_model_zone(pAfterFail, DKS_RIGHT);
        assert(pAfterFail != NULL);
        assert(pRightZoneAfterFail != NULL);
        assert(!runtime_model_subtree_contains_name(pRightZoneAfterFail, L"GLWindow"));
        DockModel_Destroy(pAfterFail);
...
static int test_runtime_command_failure_cycles_do_not_drift_gui_resources(void)
{
    DockRuntimeFixture fixture = { 0 };
    assert(runtime_fixture_init(&fixture));

    const uint32_t uIdA = 0x87000000u | (uint32_t)(GetCurrentProcessId() & 0x0000FFFFu);
    const uint32_t uIdB = uIdA + 1u;
    runtime_delete_profile_bundle(uIdA);
    runtime_delete_profile_bundle(uIdB);
    runtime_delete_window_layout_catalog_file();
...
int main(void)
{
    ...
    failed |= test_runtime_repeated_mixed_layout_cycles_with_session_restore_remain_stable();
    failed |= test_runtime_repeated_direct_floating_restore_cycles_remain_stable();
    failed |= test_runtime_repeated_direct_floating_failure_cycles_remain_stable();
    failed |= test_runtime_mixed_layout_cycles_do_not_drift_gui_resources();
    ...
    failed |= test_runtime_repeated_command_failure_cycles_from_mixed_state_remain_stable();
    failed |= test_runtime_command_failure_cycles_do_not_drift_gui_resources();
    failed |= test_runtime_menu_command_saves_and_overwrites_named_layout();
    failed |= test_runtime_repeated_menu_command_save_overwrite_cycles_remain_stable();
    failed |= test_runtime_repeated_save_apply_reset_cycles_remain_stable();
    failed |= test_runtime_save_apply_reset_cycles_do_not_drift_gui_resources();
